...
public class PokemonDao : IPokemonDao
{
    ...
    public List<float> GetDamage(int attackerId, int targetId, int moveId)
    {
        ...
        Type moveType = _pokemonTypeDao.Get(move.Type.id);
        //calculate damage multiplier
        float damageMultiplier = 1;

        foreach (Type type in target.Types!)
        {
        ...
        float damagedealt = (2 * 50 / 5f + 2) * move.Power * (attacker.Stats!.Attack / (float)target.Stats!.Defense) / 50f * damageMultiplier;
